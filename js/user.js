...
async function fetchTotalPages() {
    ...
    const response = await fetch('https://vvmm.onrender.com/nguoi-dung/count');
    const data = await response.json();
    totalPages = Math.ceil(data.result / limit);
    sumCode.innerHTML = `tổng số người tham gia: ${data.result}`
    renderPagination();
}

async function fetchUsers(page) {
    const response = await fetch(`https://vvmm.onrender.com/nguoi-dung/lay-danh-sach-nguoi-dung/?page=${page}&limit=${limit}`);
    const data = await response.json();
    renderTable(data);
}
